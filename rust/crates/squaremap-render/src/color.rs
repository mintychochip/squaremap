...
pub fn remove_alpha(color: u32) -> u32 {
    0xFF00_0000 | (color & 0x00FF_FFFF)
}

pub fn shade_level(color: u32, level: u8) -> Result<u32, ColorError> {
    let factor = match level {
        0 => 180.0_f32 / 255.0_f32,
        1 => 220.0_f32 / 255.0_f32,
        2 => 1.0,
        _ => return Err(ColorError::InvalidShade),
    };
...
pub fn shade_factor(color: u32, factor: f32) -> Result<u32, ColorError> {
    if !factor.is_finite() {
        return Err(ColorError::NonFiniteFloat);
    }
    if !(0.0..=1.0).contains(&factor) {
        return Err(ColorError::OutOfRangeFactor);
    }
...
pub fn checkerboard_parity(img_x: i32, img_z: i32) -> bool {
    (img_x.wrapping_add(img_z) & 1) != 0
}

pub fn terrain(current_y: i32, previous_y: i32, color: u32, odd: bool) -> u32 {
    let diff = (f64::from(current_y) - f64::from(previous_y))
        + (if odd { 1.0_f64 } else { 0.0 } - 0.5) * 0.4;
    let level = if diff > 0.6 {
        2
    } else if diff < -0.6 {
        0
    } else {
...
pub fn glass_composite(under: u32, glass: u32, alpha: f32) -> Result<u32, ColorError> {
    mix(under, glass, alpha)
}

pub fn average_argb(colors: &[u32]) -> Result<u32, ColorError> {
    if colors.is_empty() {
        return Err(ColorError::EmptyBlend);
    }
    ...
    for &color in colors {
        ...
        for (sum, channel) in sums.iter_mut().zip(channels) {
            *sum = sum
                ...
                .ok_or(ColorError::AccumulationOverflow)?;
            if *sum > i32::MAX as u64 {
                return Err(ColorError::AccumulationOverflow);
            }
        }
    }
    let n = colors.len() as u64;
    Ok(((sums[0] / n) as u32) << 24
        | ((sums[1] / n) as u32) << 16
        | ((sums[2] / n) as u32) << 8
        | (sums[3] / n) as u32)
}

pub fn water_biome_blend(base: u32, samples: &[u32]) -> Result<u32, ColorError> {
    mix(base, average_argb(samples)?, 0.8_f32)
}
...
pub fn compose_fluid(
    ...
    odd: bool,
    flags: FluidFlags,
) -> Result<u32, ColorError> {
    if !(1..=11).contains(&depth) {
        return Err(ColorError::InvalidDepth);
    }
    let mut shaded = false;
    match kind {
        FluidKind::Water => {
            if flags.water_checkerboard {
                color = depth_checkerboard(depth, color, odd);
                shaded = true;
            }
            if flags.water_clear {
                if !flags.water_checkerboard {
                    color = shade_factor(color, 0.85_f32 - f32::from(depth) * 0.01_f32)?;
                }
...
pub fn classify_unknown_fluid(
    rendered_color: u32,
    native_is_water: bool,
    native_is_lava: bool,
) -> FluidKind {
    if native_is_water {
        FluidKind::Water
    } else if native_is_lava {
        FluidKind::Lava
    } else if (rendered_color >> 24) & 0xff == 0xff {
        FluidKind::Lava
    } else {
        FluidKind::Water
    }
