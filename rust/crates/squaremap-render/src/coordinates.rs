...
pub fn block_to_chunk(value: i32) -> i32 {
    value.div_euclid(BLOCKS_PER_CHUNK)
}
pub fn block_to_region(value: i32) -> i32 {
    value.div_euclid(BLOCKS_PER_REGION)
}
pub fn chunk_to_region(value: i32) -> i32 {
    value.div_euclid(CHUNKS_PER_REGION)
}
pub fn chunk_to_block(value: i32) -> Result<i32, CoordinateError> {
    checked_mul(value, BLOCKS_PER_CHUNK)
}
pub fn region_to_chunk(value: i32) -> Result<i32, CoordinateError> {
    checked_mul(value, CHUNKS_PER_REGION)
}
pub fn region_to_block(value: i32) -> Result<i32, CoordinateError> {
    checked_mul(value, BLOCKS_PER_REGION)
}
...
impl ChunkCoord {
    pub fn region(self) -> RegionCoord {
        RegionCoord {
            x: chunk_to_region(self.x),
            z: chunk_to_region(self.z),
        }
    }
...
fn validate_zoom(zoom: u8, max_zoom: u8) -> Result<(), CoordinateError> {
    if zoom > max_zoom || max_zoom > 9 {
        Err(CoordinateError::InvalidZoom)
    } else {
        Ok(())
    }
}

pub fn tile_for_region(
    region_x: i32,
    region_z: i32,
    zoom: u8,
    max_zoom: u8,
) -> Result<TileCoord, CoordinateError> {
...
pub fn tile_origin(region_x: i32, region_z: i32, zoom: u8) -> Result<(u16, u16), CoordinateError> {
    if zoom > 9 {
        return Err(CoordinateError::InvalidZoom);
    }
    let step = 1_i64 << zoom;
