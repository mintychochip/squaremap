...
fn replacement_views_preserve_ui_world_names_marker_shapes_timestamps_and_icon_pixels() {
    use squaremap_protocol::wire::{
        envelope, marker, Envelope, Icon, IconsReplace, Marker, MarkerLayer, MarkerLayersReplace,
        MarkerMultiPolygon, MarkerPolygon, MarkerPolyline, Point, PointList, UiSettings, World,
        WorldIdentity, WorldStateReplace,
    };
    let temp = tempfile::tempdir().unwrap();
    let root = OutputRoot::new(temp.path()).unwrap();
    let world = World {
        identity: Some(WorldIdentity { namespace: "custom".into(), value: "overworld".into(), ..Default::default() }),
    ...
    assert!(settings.contains("\"name\":\"custom_overworld\""), "{settings}");
    assert!(settings.contains("\"title\":\"Squaremap\""));
    assert!(root.latest_bytes("tiles/custom_overworld/settings.json").is_some());

    let marker = Marker {
        geometry: Some(marker::Geometry::MultiPolygon(MarkerMultiPolygon {
            polygons: vec![MarkerPolygon {
                main_polygon: vec![Point { x: 0, z: 0 }, Point { x: 2, z: 0 }, Point { x: 2, z: 2 }],
                negative_space: vec![PointList { points: vec![Point { x: 1, z: 1 }, Point { x: 1, z: 2 }] }],
            }],
        })),
        ..Default::default()
    };
    let single_line = Marker {
        geometry: Some(marker::Geometry::Polyline(MarkerPolyline {
            lines: vec![PointList { points: vec![Point { x: 3, z: 4 }, Point { x: 5, z: 6 }] }],
        })),
        ..Default::default()
    };
    let multi_line = Marker {
        geometry: Some(marker::Geometry::Polyline(MarkerPolyline {
            lines: vec![
                PointList { points: vec![Point { x: 7, z: 8 }, Point { x: 9, z: 10 }] },
                PointList { points: vec![Point { x: 11, z: 12 }, Point { x: 13, z: 14 }] },
            ],
        })),
        ..Default::default()
    };
    let polygon = Marker {
        geometry: Some(marker::Geometry::Polygon(MarkerPolygon {
            ...
            negative_space: vec![PointList { points: vec![Point { x: 19, z: 20 }, Point { x: 21, z: 22 }] }],
        })),
        ..Default::default()
    };
    let marker_envelope = Envelope {
        payload: Some(envelope::Payload::MarkerLayersReplace(MarkerLayersReplace {
            revision: 1,
            world: Some(WorldIdentity { namespace: "custom".into(), value: "overworld".into(), ..Default::default() }),
    ...
    let markers = String::from_utf8(root.latest_bytes("tiles/custom_overworld/markers.json").unwrap()).unwrap();
    ...
    let reloaded_world = Envelope {
        payload: Some(envelope::Payload::WorldStateReplace(WorldStateReplace {
            revision: 2,
            worlds: vec![World {
                identity: Some(WorldIdentity { namespace: "custom".into(), value: "overworld".into(), epoch: 1 }),
    ...
    let reloaded_settings = root.latest_bytes("tiles/custom_overworld/settings.json").unwrap();
    assert!(!reloaded_settings.is_empty());
    assert!(root.latest_bytes("tiles/custom_overworld/markers.json").is_none());

    let icon_envelope = Envelope {
        payload: Some(envelope::Payload::IconsReplace(IconsReplace {
            revision: 1,
            icons: vec![Icon { id: "pixel".into(), image: vec![255, 0, 0, 255], mime_type: "image/rgba".into(), width: 1, height: 1 }],
        })),
        ..Default::default()
    };
    ...
    let empty_icons = Envelope {
        payload: Some(envelope::Payload::IconsReplace(IconsReplace { revision: 2, icons: vec![] })),
        ..Default::default()
    };
    squaremap_server::views::apply_replacement(&root, &empty_icons).unwrap();
    assert!(!temp.path().join("images/icon/registered/pixel.png").exists());
    let empty_worlds = Envelope {
        payload: Some(envelope::Payload::WorldStateReplace(WorldStateReplace { revision: 3, worlds: vec![], ..Default::default() })),
        ..Default::default()
    };
    squaremap_server::views::apply_replacement(&root, &empty_worlds).unwrap();
    assert!(!temp.path().join("tiles/custom_overworld/settings.json").exists());
    assert!(!temp.path().join("tiles/custom_overworld/markers.json").exists());
}
#[test]
fn checked_in_legacy_fixtures_are_consumed() {
    let empty: serde_json::Value = serde_json::from_str(include_str!("../../../../testdata/bridge/v1/views/empty.json")).unwrap();
    let settings: serde_json::Value = serde_json::from_str(include_str!("../../../../testdata/bridge/v1/views/settings.json")).unwrap();
    let world_settings: serde_json::Value = serde_json::from_str(include_str!("../../../../testdata/bridge/v1/views/world-settings.json")).unwrap();
    let players: serde_json::Value = serde_json::from_str(include_str!("../../../../testdata/bridge/v1/views/players.json")).unwrap();
    let icons: serde_json::Value = serde_json::from_str(include_str!("../../../../testdata/bridge/v1/views/icons.json")).unwrap();
    let markers: serde_json::Value = serde_json::from_str(include_str!("../../../../testdata/bridge/v1/views/markers.json")).unwrap();
    let temp = tempfile::tempdir().unwrap();
    let root = OutputRoot::new(temp.path()).unwrap();
    let production_icons = squaremap_protocol::wire::Envelope {
        payload: Some(squaremap_protocol::wire::envelope::Payload::IconsReplace(squaremap_protocol::wire::IconsReplace {
            revision: 1,
    ...
    assert_eq!(marker_list.iter().map(|marker| marker["type"].as_str().unwrap()).collect::<Vec<_>>(),
        vec!["icon", "circle", "ellipse", "rectangle", "polyline", "polyline", "polygon", "polygon"]);
...
fn decode_rgba_png(bytes: &[u8]) -> (u32, u32, Vec<u8>) {
    ...
    while compressed + 5 <= idat.len() - 4 {
        ...
        let length = u16::from_le_bytes(idat[compressed + 1..compressed + 3].try_into().unwrap()) as usize;
...
fn replacement_views_sort_worlds_players_markers_and_icons() {
    use squaremap_protocol::wire::{
        envelope, Envelope, Icon, IconsReplace, MarkerLayer, MarkerLayersReplace, Player, PlayersReplace,
        World, WorldIdentity, WorldStateReplace,
    };
    let temp = tempfile::tempdir().unwrap();
    let root = OutputRoot::new(temp.path()).unwrap();
    let worlds = Envelope {
        payload: Some(envelope::Payload::WorldStateReplace(WorldStateReplace {
            revision: 1,
            worlds: vec![
                World { identity: Some(WorldIdentity { namespace: "z".into(), value: "world".into(), ..Default::default() }), ..Default::default() },
                World { identity: Some(WorldIdentity { namespace: "a".into(), value: "world".into(), ..Default::default() }), ..Default::default() },
            ],
            ..Default::default()
        })),
        ..Default::default()
    ...
    let settings = String::from_utf8(root.latest_bytes("tiles/settings.json").unwrap()).unwrap();
    assert!(settings.find("\"name\":\"a_world\"").unwrap() < settings.find("\"name\":\"z_world\"").unwrap());

    let markers = Envelope {
        payload: Some(envelope::Payload::MarkerLayersReplace(MarkerLayersReplace {
            revision: 1,
            world: Some(WorldIdentity { namespace: "a".into(), value: "world".into(), ..Default::default() }),
            layers: vec![MarkerLayer { id: "z".into(), ..Default::default() }, MarkerLayer { id: "a".into(), ..Default::default() }],
        })),
        ..Default::default()
    };
    squaremap_server::views::apply_replacement(&root, &markers).unwrap();
    let marker_json = String::from_utf8(root.latest_bytes("tiles/a_world/markers.json").unwrap()).unwrap();
    ...
    let icons = Envelope {
        payload: Some(envelope::Payload::IconsReplace(IconsReplace {
            revision: 1,
            icons: vec![
                Icon { id: "z".into(), image: vec![0, 0, 0, 255], width: 1, height: 1, ..Default::default() },
                Icon { id: "a".into(), image: vec![0, 0, 0, 255], width: 1, height: 1, ..Default::default() },
            ],
        })),
        ..Default::default()
    };
    ...
    let players = Envelope {
        payload: Some(envelope::Payload::PlayersReplace(PlayersReplace {
            revision: 1,
            players: vec![
                Player { uuid: vec![255], world: Some(WorldIdentity { namespace: "a".into(), value: "world".into(), ..Default::default() }), ..Default::default() },
                Player { uuid: vec![0], world: Some(WorldIdentity { namespace: "a".into(), value: "world".into(), ..Default::default() }), ..Default::default() },
            ],
            ..Default::default()
        })),
        ..Default::default()
    ...
    assert!(player_json.find("\"uuid\":\"00\"").unwrap() < player_json.find("\"uuid\":\"ff\"").unwrap());
}
#[test]
fn unsupported_and_invalid_replacements_do_not_commit_state() {
    use squaremap_protocol::wire::{envelope, ConfigReplace, Envelope, Marker, MarkerLayer, MarkerLayersReplace, WorldIdentity};
    let temp = tempfile::tempdir().unwrap();
    let root = OutputRoot::new(temp.path()).unwrap();
    ...
    let invalid = Envelope {
        payload: Some(envelope::Payload::MarkerLayersReplace(MarkerLayersReplace {
            revision: 1,
            world: Some(WorldIdentity { namespace: "minecraft".into(), value: "overworld".into(), ..Default::default() }),
            layers: vec![MarkerLayer { markers: vec![Marker::default()], ..Default::default() }],
        })),
        ..Default::default()
    };
    ...
    assert!(root.latest_bytes("tiles/minecraft_overworld/markers.json").is_none());
}
#[test]
fn initial_out_of_order_replacements_converge_and_epoch_validation_is_atomic() {
    use squaremap_protocol::wire::{
        envelope, Envelope, MarkerLayer, MarkerLayersReplace, Player, PlayersReplace, World, WorldIdentity,
        WorldStateReplace,
    };
    let temp = tempfile::tempdir().unwrap();
    let root = OutputRoot::new(temp.path()).unwrap();
    let identity = WorldIdentity { namespace: "custom".into(), value: "overworld".into(), epoch: 9 };
    let players = Envelope {
        payload: Some(envelope::Payload::PlayersReplace(PlayersReplace {
            revision: 1,
            players: vec![Player { uuid: vec![1], world: Some(identity.clone()), ..Default::default() }],
            ..Default::default()
        })),
        ..Default::default()
    };
    ...
    let markers = Envelope {
        payload: Some(envelope::Payload::MarkerLayersReplace(MarkerLayersReplace {
            ...
            layers: vec![MarkerLayer { id: "layer".into(), ..Default::default() }],
        })),
        ..Default::default()
    };
    squaremap_server::views::apply_replacement(&root, &markers).unwrap();
    let worlds = Envelope {
        payload: Some(envelope::Payload::WorldStateReplace(WorldStateReplace {
            revision: 1,
            worlds: vec![World { identity: Some(identity.clone()), ..Default::default() }],
            ..Default::default()
        })),
        ..Default::default()
    };
    ...
    let before = root.latest_bytes("tiles/custom_overworld/markers.json").unwrap();
    let invalid_players = Envelope {
        payload: Some(envelope::Payload::PlayersReplace(PlayersReplace {
            revision: 2,
            players: vec![Player {
                uuid: vec![1],
                world: Some(WorldIdentity { epoch: 10, ..identity.clone() }),
                ..Default::default()
            }],
            ..Default::default()
        })),
    ...
    assert_eq!(root.latest_bytes("tiles/players.json").unwrap(), root.canonical_state().unwrap().players);
    assert_eq!(root.latest_bytes("tiles/custom_overworld/markers.json").unwrap(), before);

    let newer_world = Envelope {
        payload: Some(envelope::Payload::WorldStateReplace(WorldStateReplace {
            revision: 2,
            worlds: vec![World {
                identity: Some(WorldIdentity { epoch: 10, ..identity }),
                ..Default::default()
            }],
            ..Default::default()
        })),
    ...
    assert!(!temp.path().join("tiles/custom_overworld/markers.json").exists());
    assert!(!root.canonical_state().unwrap().marker_revisions.contains_key("custom_overworld"));
}


#[test]
fn replacement_reconciles_files_left_by_an_existing_process() {
    use squaremap_protocol::wire::{envelope, Envelope, Icon, IconsReplace, World, WorldIdentity, WorldStateReplace};
    let temp = tempfile::tempdir().unwrap();
    {
        let root = OutputRoot::new(temp.path()).unwrap();
        root.atomic_write("images/icon/registered/stale.png", b"stale").unwrap();
        root.atomic_write("tiles/old_world/settings.json", b"stale").unwrap();
        root.atomic_write("tiles/old_world/markers.json", b"stale").unwrap();
    }
    let root = OutputRoot::new(temp.path()).unwrap();
    let icons = Envelope {
        payload: Some(envelope::Payload::IconsReplace(IconsReplace {
            revision: 1,
            icons: vec![Icon { id: "active".into(), image: vec![1, 2, 3, 255], width: 1, height: 1, ..Default::default() }],
        })),
        ..Default::default()
    };
    squaremap_server::views::apply_replacement(&root, &icons).unwrap();
    assert!(!temp.path().join("images/icon/registered/stale.png").exists());
    let worlds = Envelope {
        payload: Some(envelope::Payload::WorldStateReplace(WorldStateReplace {
            revision: 1,
            worlds: vec![World { identity: Some(WorldIdentity { namespace: "new".into(), value: "world".into(), epoch: 1 }), ..Default::default() }],
            ..Default::default()
        })),
        ..Default::default()
    };
...
fn decode_single_pixel(png: &[u8]) -> [u8; 4] {
    let mut offset = 8;
