...
async fn proxies_http_and_keeps_excluded_paths_local() {
    ...
    let local = client
        .get(format!("{base}/tiles/local.png"))
        .send()
        .await
        .unwrap();
...
async fn websocket_forwards_headers_and_selected_protocol() {
    ...
    let upstream_task = tokio::spawn(async move {
        ...
        let _ = tokio_tungstenite::accept_hdr_async(stream, callback)
            .await
            .unwrap();
    });
    let dir = tempdir().unwrap();
...
async fn shutdown_kills_descendant_process_group() {
    ...
    let script = format!(
        "trap '' TERM; sh -c 'trap \"\" TERM; while :; do sleep 1; done' & child=$!; echo $child > '{}'; printf 'http://127.0.0.1:9\\n'; wait",
        pid_file.display()
    );
    let fake = executable(dir.path(), &script);
    let root = OutputRoot::new(dir.path()).unwrap();
...
async fn startup_failure_kills_saved_process_group_descendant() {
    ...
    let script = format!(
        "sh -c 'trap \"\" TERM; while :; do sleep 1; done' & child=$!; echo $child > '{}'; printf 'http://127.0.0.1:9\\n'; exit 0",
        pid_file.display()
    );
    let root = OutputRoot::new(dir.path()).unwrap();
    let fake = executable(dir.path(), &script);
