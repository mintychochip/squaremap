use squaremap_protocol::wire::{
    BackendResultCode, BackendSubstitution, ControlKind, ControlRequest, ControlResult,
    WorldIdentity, backend_substitution,
};
...
pub struct ControlState {
    in_config: bool,
    worlds: HashSet<(String, String, u64)>,
}
...
impl ControlState {
    ...
    pub fn handle(&mut self, request: &ControlRequest) -> ControlResult {
        let Some(kind) = ControlKind::try_from(request.kind).ok() else {
            return result(BackendResultCode::InvalidRequest, None);
        };
        if kind == ControlKind::Health {
            return result(
                if self.in_config {
                    BackendResultCode::Healthy
                } else {
        ...
        if !self.worlds.contains(&key(identity)) {
            return result(BackendResultCode::UnknownWorld, Some(identity));
        }
        match kind {
            ControlKind::FullRender
            | ControlKind::RadiusRender
            | ControlKind::CancelRender
            | ControlKind::PauseRenders
            | ControlKind::ResetMap => {
                result(BackendResultCode::BackendUnavailable, Some(identity))
            }
            ControlKind::Health | ControlKind::Reload | ControlKind::Unspecified => {
                result(BackendResultCode::InvalidRequest, Some(identity))
            }
        }
    }
}
fn key(identity: &WorldIdentity) -> (String, String, u64) {
    (
...
mod tests {
    use super::*;
    fn world(namespace: &str, value: &str, epoch: u64) -> WorldIdentity {
        WorldIdentity {
    ...
    fn request(kind: ControlKind, identity: Option<WorldIdentity>) -> ControlRequest {
        ControlRequest {
            kind: kind as i32,
            world: identity,
            center_x: 0,
    ...
    fn controls_are_unavailable_without_fake_state_mutation() {
        ...
        for kind in [
            ...
            assert_eq!(
                state.handle(&request(kind, Some(configured.clone()))).code,
                BackendResultCode::BackendUnavailable as i32
            );
        }
    ...
    fn health_requires_an_accepted_configuration() {
        ...
        let mut state = ControlState::default();
        assert_eq!(
            state.handle(&health).code,
            BackendResultCode::BackendUnavailable as i32
        );
        state.replace_worlds(vec![world("minecraft", "overworld", 1)]);
        assert_eq!(
            state.handle(&health).code,
            BackendResultCode::Healthy as i32
        );
    }
    ...
    fn accepted_world_epoch_is_resolved_and_unknown_world_is_rejected() {
        ...
        assert_eq!(
            state
                .handle(&request(ControlKind::FullRender, Some(configured)))
                .code,
            BackendResultCode::BackendUnavailable as i32
        );
        assert_eq!(
            state
                .handle(&request(
                    ControlKind::FullRender,
                    Some(world("minecraft", "overworld", 0))
                ))
                .code,
            BackendResultCode::UnknownWorld as i32
        );
    }
    ...
    fn accepted_config_replaces_old_world_table() {
        ...
        assert_eq!(
            state
                .handle(&request(ControlKind::FullRender, Some(first)))
                .code,
            BackendResultCode::UnknownWorld as i32
        );
        assert_eq!(
            state
                .handle(&request(ControlKind::FullRender, Some(second)))
                .code,
            BackendResultCode::BackendUnavailable as i32
        );
    }
    ...
    fn reload_does_not_fake_success_without_java_reload_executor() {
        ...
        state.replace_worlds(vec![world("minecraft", "overworld", 1)]);
        assert_eq!(
            state.handle(&request(ControlKind::Reload, None)).code,
            BackendResultCode::BackendUnavailable as i32
        );
    }
}
