use super::{InstallRequest, StagedInstall, TileInstaller};
use async_trait::async_trait;
use squaremap_render::coordinates::{CHUNKS_PER_REGION, ChunkCoord};
use squaremap_render::{
    BiomeSource, PngOptions, PublishResult, RegionPixels, RenderContext, RenderSettings,
    SnapshotBiomeSource, TilePyramid, TileStore, TileStoreError, render_chunk,
};
...
impl PrefixedTileStore {
    pub fn new(prefix: impl Into<PathBuf>, inner: Arc<dyn TileStore>) -> Result<Self, String> {
        ...
        if prefix.is_absolute() {
            return Err("tile prefix must be relative".into());
        }
...
impl RenderTileInstaller {
    ...
    pub fn configure_world(
        ...
        let world_store = Arc::new(PrefixedTileStore::new(
            config.tile_prefix.clone(),
            self.store.clone(),
        )?);
        let pyramid = TilePyramid::new(world_store, config.max_zoom, config.png_options)
            .map_err(|error| error.to_string())?;
        self.worlds
            .write()
            .map_err(|_| "world render config lock poisoned")?
            .insert(world.clone(), config);
        self.pyramids
            .write()
            .map_err(|_| "tile pyramid lock poisoned")?
            .insert(world, Arc::new(pyramid));
        Ok(())
    }
}

struct StagedTilePyramidInstall {
    pyramid: Arc<TilePyramid>,
    staged: squaremap_render::StagedRegion,
}

#[async_trait]
impl StagedInstall for StagedTilePyramidInstall {
    async fn publish(self: Box<Self>) -> Result<(), String> {
        self.pyramid
            .publish_staged(self.staged)
            .await
            .map_err(|error| error.to_string())?;
        Ok(())
    }
...
impl TileInstaller for RenderTileInstaller {
    async fn stage(&self, request: InstallRequest) -> Result<Box<dyn StagedInstall>, String> {
        let config = self
            ...
            .get(&request.world)
            .cloned()
            .ok_or_else(|| "world render config is unavailable".to_string())?;
        let pyramid = self
            .pyramids
            .read()
            .map_err(|_| "tile pyramid lock poisoned")?
            .get(&request.world)
            .cloned()
            .ok_or_else(|| "world tile pyramid is unavailable".to_string())?;
        ...
        let biome_source: Option<Arc<dyn BiomeSource>> = if config.settings.biomes_enabled {
            let mut source = SnapshotBiomeSource::new(generation.clone(), config.biome_zoom_seed)
                .with_snapshot(request.snapshots.center.clone())
                .map_err(|error| error.to_string())?;
        ...
        for x in 0..16 {
            for z in 0..16 {
                pixels
                    .set_argb(local_x + x, local_z + z, rendered.pixel(x, z))
                    .map_err(|error| error.to_string())?;
            }
        }
        let staged = pyramid
            .stage_region(region, &pixels)
            .await
            .map_err(|error| error.to_string())?;
        Ok(Box::new(StagedTilePyramidInstall { pyramid, staged }))
    }
}
