    ...
    match serde_json::from_slice(&job.payload) {
        ...
        Err(error) if job.kind == JobKind::Resume => {
            ...
            Ok(JobCursor {
                coordinates: entries
                    .into_iter()
                    .filter_map(|LegacyResumeEntry(coordinate, completed)| {
                        (!completed).then_some(coordinate)
                    })
                    .collect(),
                next: 0,
                revision: 0,
...
pub(super) async fn start(
    ...
    if !matches!(kind, JobKind::Full | JobKind::Radius) {
        return Err(SchedulerError::InvalidJob(
            "scheduler can only start full or radius jobs",
        ));
    }
    if coordinates.is_empty() {
        return Err(SchedulerError::InvalidJob(
            "render job requires at least one coordinate",
        ));
    }
...
pub(super) async fn run(
    ...
    let mut job = scheduler
        ...
        .load_render_job(id)
        .await?
        .ok_or(SchedulerError::MissingJob)?;
    ...
    if cursor.next > cursor.coordinates.len() || job.completed_chunks != cursor.next as u64 {
        return Err(SchedulerError::InvalidJob(
            "render job cursor and completed count disagree",
        ));
    }
    ...
    while cursor.next < cursor.coordinates.len() && report.selected < max_steps {
        if !scheduler.wait_unpaused(&job.world, Some(id)).await {
...
pub(super) async fn cancel(scheduler: &Scheduler, id: &[u8]) -> Result<(), SchedulerError> {
    ...
    let job = scheduler
        ...
        .await?
        .ok_or(SchedulerError::MissingJob)?;
    scheduler.invalidate_install_fence(&job.world).await;
    let mut job = job;
    if !matches!(
        job.state,
