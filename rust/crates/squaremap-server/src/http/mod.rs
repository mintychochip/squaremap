...
async fn handle_request(
    ...
    let proxy = state
        .dev
        .as_ref()
        .is_some_and(|_| !is_static_exclusion(&decoded));
    if proxy {
        if let Some(dev) = state.dev.as_ref() {
            return dev.proxy(request).await.unwrap_or_else(|_| {
                response(StatusCode::BAD_GATEWAY, HeaderMap::new(), Body::empty())
            });
        }
    }
...
fn is_static_exclusion(path: &std::path::Path) -> bool {
    ...
    if first == "images"
        && components.next() == Some(std::path::Component::Normal("icon".as_ref()))
        && components.next() == Some(std::path::Component::Normal("registered".as_ref()))
    {
        return true;
    }
    false
...
pub(crate) fn make_response(status: StatusCode, headers: HeaderMap, body: Body) -> Response<Body> {
