...
use squaremap_protocol::wire::{
    envelope, marker, Envelope, Marker, MarkerLayer, Point, World, WorldIdentity, WorldStateReplace,
};
use squaremap_state::view::{
    serialize_json, IconView, IconsView, MarkerGeometryView, MarkerLayerView, MarkerStyleView,
    MarkerTooltipView, MarkerView, PlayersView, PlayerView, PolylinePoints, SettingsView,
    UiCoordinatesView, UiLinkView, UiSidebarView, UiView, ViewPoint, WorldSettingsView,
    WorldSummaryView, NameplatesView, PlayerTrackerView, SpawnView, ZoomView,
};
...
pub fn write_world_settings(root: &OutputRoot, world: &str, view: &WorldSettingsView) -> io::Result<Vec<u8>> {
...
pub fn apply_replacement(root: &OutputRoot, envelope: &Envelope) -> io::Result<Option<Vec<u8>>> {
    match envelope.payload.as_ref() {
        Some(envelope::Payload::PlayersReplace(value)) => {
            ...
            let bytes = write_players(root, &PlayersView { players, max: value.max_players })?;
        ...
        Some(envelope::Payload::MarkerLayersReplace(value)) => {
            let world = value.world.as_ref().map(legacy_world_identity_name).unwrap_or_else(|| "world".into());
            let mut state = root.canonical_state()?;
            reject_stale(value.revision, state.marker_revisions.get(&world).copied().unwrap_or(0), "markers")?;
            if let Some(identity) = value.world.as_ref() {
                validate_world_epoch(&state, identity)?;
            } else if !state.world_epochs.is_empty() {
                return Err(io::Error::new(io::ErrorKind::InvalidData, "marker replacement has no world identity"));
            }
            let mut ordered_layers = value.layers.iter().collect::<Vec<_>>();
            ordered_layers.sort_by(|left, right| left.id.cmp(&right.id));
            let layers: Vec<MarkerLayerView> = ordered_layers.into_iter().map(marker_layer).collect::<io::Result<Vec<_>>>()?;
            let bytes = write_json(root, &format!("tiles/{world}/markers.json"), &layers)?;
            state.marker_revisions.insert(world.clone(), value.revision);
            state.marker_outputs.insert(format!("tiles/{world}/markers.json"), bytes.clone());
            root.replace_canonical(state)?;
            Ok(Some(bytes))
        }
        Some(envelope::Payload::IconsReplace(value)) => {
            ...
            let encoded = ordered_icons.iter().map(|icon| {
                let path = format!("images/icon/registered/{}.png", icon.id);
                let bytes = encode_rgba_png(icon.width, icon.height, &icon.image)
                    .map_err(|error| io::Error::new(io::ErrorKind::InvalidData, error))?;
                let view = IconView { id: icon.id.clone(), mime_type: icon.mime_type.clone(), width: icon.width, height: icon.height };
                Ok((path, bytes, view))
            ...
            for path in state.icon_assets.keys().filter(|path| !current.contains_key(*path)).cloned().collect::<Vec<_>>() {
                root.remove(path)?;
            }
        ...
        Some(envelope::Payload::WorldStateReplace(value)) => {
            ...
            let reloaded_worlds = worlds.iter().filter_map(|world| {
                let identity = world.identity.as_ref()?;
                let key = format!("{}/{}", identity.namespace, identity.value);
                (state.world_epochs.get(&key).is_some_and(|previous| identity.epoch > *previous)).then(|| world_name(world))
            }).collect::<Vec<_>>();
            ...
            let current_worlds: std::collections::HashSet<String> = worlds.iter().map(|world| world_name(world)).collect();
            for path in existing_tiles.iter().filter(|path| {
                path.to_str().and_then(|value| value.strip_prefix("tiles/")).and_then(|value| value.split_once('/'))
                    .is_some_and(|(name, file)| (file == "markers.json" && reloaded_worlds.iter().any(|reloaded| reloaded == name))
                        || ((file == "settings.json" || file == "markers.json") && !current_worlds.contains(name)))
            }) {
                root.remove(path)?;
            }
            for path in state.marker_outputs.keys().filter(|path| {
                path.strip_prefix("tiles/").and_then(|value| value.strip_suffix("/markers.json"))
            ...
            for path in state.world_outputs.keys().filter(|path| !outputs.contains_key(*path)).cloned().collect::<Vec<_>>() {
                root.remove(path)?;
            }
            state.world_outputs = outputs;
            state.world_epochs = worlds.iter().filter_map(|world| world.identity.as_ref().map(|id| (format!("{}/{}", id.namespace, id.value), id.epoch))).collect();
            state.worlds_revision = value.revision;
            state.worlds = bytes.clone();
            root.replace_canonical(state)?;
            Ok(Some(bytes))
        }
        _ => Err(io::Error::new(io::ErrorKind::Unsupported, "unsupported bridge payload")),
    }
}

fn validate_world_epoch(state: &squaremap_state::CanonicalState, identity: &WorldIdentity) -> io::Result<()> {
    ...
    match state.world_epochs.get(&key) {
        ...
        Some(_) => Err(io::Error::new(io::ErrorKind::InvalidData, "world epoch does not match canonical world")),
        None => Err(io::Error::new(io::ErrorKind::InvalidData, "world is not present in canonical state")),
    }
}

fn validate_player_epoch(state: &squaremap_state::CanonicalState, player: &squaremap_protocol::wire::Player) -> io::Result<()> {
    if state.world_epochs.is_empty() {
        return Ok(());
    }
    let identity = player.world.as_ref().ok_or_else(|| io::Error::new(io::ErrorKind::InvalidData, "player has no world identity"))?;
    validate_world_epoch(state, identity)
}
...
fn stale_error(domain: &str) -> io::Error {
    io::Error::new(io::ErrorKind::InvalidData, format!("stale {domain} replacement"))
}
...
fn player(value: &squaremap_protocol::wire::Player) -> PlayerView {
    PlayerView {
        ...
        world: value.world.as_ref().map(legacy_world_identity_name).unwrap_or_default(),
        x: value.x,
        y: value.y,
        z: value.z,
        yaw: value.yaw,
...
fn sorted_worlds(value: &WorldStateReplace) -> Vec<&World> {
    ...
    worlds.sort_by(|left, right| {
        ...
        left_identity.map(|id| id.namespace.as_str()).unwrap_or("")
            .cmp(right_identity.map(|id| id.namespace.as_str()).unwrap_or(""))
            .then_with(|| left_identity.map(|id| id.value.as_str()).unwrap_or("")
...
fn settings(value: &WorldStateReplace, worlds: &[&World]) -> SettingsView {
    SettingsView {
        worlds: worlds.iter().map(|world| WorldSummaryView {
        ...
        ui: value.ui.as_ref().map_or_else(|| UiView {
            title: String::new(),
            coordinates: UiCoordinatesView { enabled: false, html: String::new() },
            ...
            coordinates: UiCoordinatesView { enabled: ui.coordinates_enabled, html: ui.coordinates_html.clone() },
            link: UiLinkView { enabled: ui.link_enabled },
            sidebar: UiSidebarView {
...
fn world_settings(world: &World) -> WorldSettingsView {
    ...
    let nameplates = NameplatesView {
        ...
        heads_url: tracker.map(|v| v.nameplate_heads_url.clone()).unwrap_or_default(),
    ...
    WorldSettingsView {
        spawn: world.spawn.as_ref().map(|v| SpawnView { x: v.x, z: v.z }).unwrap_or(SpawnView { x: 0, z: 0 }),
...
fn world_name(world: &World) -> String {
    world.identity.as_ref().map(legacy_world_identity_name).unwrap_or_default()
}
...
fn marker_layer(layer: &MarkerLayer) -> io::Result<MarkerLayerView> {
    Ok(MarkerLayerView {
        ...
        markers: layer.markers.iter().map(marker_view).collect::<io::Result<Vec<_>>>()?,
    })
}

fn marker_view(value: &Marker) -> io::Result<MarkerView> {
    let style = value.style.as_ref();
    let geometry = value.geometry.as_ref().map(geometry).transpose()?.ok_or_else(|| {
        io::Error::new(io::ErrorKind::InvalidData, "marker geometry not set")
    })?;
    Ok(MarkerView {
        style: MarkerStyleView {
            stroke: style.map(|v| v.stroke).unwrap_or(true),
            color: style.map(|v| v.stroke_color.clone()).unwrap_or_else(|| "#0000ff".into()),
            ...
            fill_rule: style.map(|v| v.fill_rule.clone()).unwrap_or_else(|| "evenodd".into()),
        },
        tooltip: value.tooltip.as_ref().map(|tooltip| MarkerTooltipView {
            click: tooltip.click.clone(),
            hover: tooltip.hover.clone(),
        }).and_then(|tooltip| if tooltip.click.is_some() || tooltip.hover.is_some() { Some(tooltip) } else { None }),
        geometry,
    })
}
fn geometry(value: &marker::Geometry) -> io::Result<MarkerGeometryView> {
    Ok(match value {
        marker::Geometry::Icon(value) => MarkerGeometryView::Icon {
            ...
            size: ViewPoint { x: value.size_x, z: value.size_z },
        ...
        marker::Geometry::Circle(value) => MarkerGeometryView::Circle {
            center: point(value.center.as_ref()), radius: value.radius,
        },
        marker::Geometry::Ellipse(value) => MarkerGeometryView::Ellipse {
            center: point(value.center.as_ref()), radius_x: value.radius_x, radius_z: value.radius_z,
        },
        ...
        marker::Geometry::Polyline(value) => {
            let lines = value.lines.iter().map(|line| line.points.iter().map(|p| point(Some(p))).collect()).collect::<Vec<Vec<ViewPoint>>>();
        ...
        marker::Geometry::Polygon(value) => MarkerGeometryView::Polygon { points: polygon_points(value) },
...
fn polygon_points(value: &squaremap_protocol::wire::MarkerPolygon) -> Vec<Vec<ViewPoint>> {
    ...
    points.extend(value.negative_space.iter().map(|line| line.points.iter().map(|p| point(Some(p))).collect()));
    points
}
fn encode_rgba_png(width: u32, height: u32, rgba: &[u8]) -> Result<Vec<u8>, &'static str> {
    ...
    let row = usize::try_from(width).ok().and_then(|value| value.checked_mul(4)).ok_or("icon dimensions overflow")?;
    let expected = row.checked_mul(usize::try_from(height).map_err(|_| "icon dimensions overflow")?).ok_or("icon dimensions overflow")?;
...
fn point(value: Option<&Point>) -> ViewPoint {
    value.map(|value| ViewPoint { x: value.x, z: value.z }).unwrap_or(ViewPoint { x: 0, z: 0 })
}

fn write_json<T: serde::Serialize>(root: &OutputRoot, path: &str, view: &T) -> io::Result<Vec<u8>> {
    let bytes = serialize_json(view).map_err(|error| io::Error::new(io::ErrorKind::InvalidData, error))?;
    root.atomic_write(path, &bytes)?;
    Ok(bytes)
}
