    ...
    if command == "serve-fixture" {
        ...
        while let Some(argument) = args.next() {
            match argument.as_str() {
                ...
                other => {
                    eprintln!("unknown serve-fixture argument: {other}");
                    std::process::exit(2);
                }
            }
        }
        let Some(root) = root else {
            eprintln!("serve-fixture requires --root");
            std::process::exit(2);
        };
        let Some(bind) = bind else {
            eprintln!("serve-fixture requires --bind");
            std::process::exit(2);
        };
        ...
        let result = runtime.block_on(async move {
            ...
            println!(
                "READY http_addr={}",
                server.local_addr().expect("enabled server has address")
            );
