...
- Review regressions were first run against commit `f7e6367` before implementation: the expanded recovery suite ran 10 tests and failed 6 (malformed schema mutation, epoch tombstone, duplicate resume/symlink, conflicting job ID/malformed recovery blob, epoch-qualified markers, and live-job preservation). After the fixes, the same recovery suite passed 10/10, and the focused session suite passed 12/12.
